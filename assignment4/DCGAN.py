...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=200, n_batch=128):
    ...
    for i in range(n_epochs):
        ...
        xr, _ = generate_real_samples(dataset, half_batch)
        xf, _ = generate_fake_samples(g_model, latent_dim, half_batch)
        s, m = image_comparison(xr,xf)
        performance[i] = {'ssim': str(s), 'mse': str(m)}
...
# train model
train(g_model, d_model, gan_model, dataset, latent_dim, 100, 256)

#g_model = load_model('generator_model.h5')
#g_model.summary()
#x, y = generate_fake_samples(g_model, latent_dim, 50000)
#save_plot(x, 0, 223)
